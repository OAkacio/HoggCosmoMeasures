...
try:
    obsdatalist = sl.loadtable("obs_data.txt", sep=sl.identificadora("obs_data.txt"))
    ...
    sy.status(f"Um erro foi encontrado ao tentar ler os dados observados. Erro: {e}")
...
sy.param(
    ...
    ("Densidade de materia Bestfit (2dBF_Omega_M)", omegaM_list[MINDchi2d_min], "adm."),
    (
...
sy.ok(one=True, lista=("Gráfico de Distribuição de Qui-Quadrado"))

# ? -----------------------------------------------------------------------------
# ?         QUEBRA DE DEGENERECÊNCIA (PRIOR DA CMB)
# ? -----------------------------------------------------------------------------

sy.status("Iniciando quebra de degenerescência: Aplicando Prior da CMB na malha 2D...")
