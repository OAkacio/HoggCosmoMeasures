...
def plot(
    ...
    fig, ax = plt.subplots(figsize=(8, 6), dpi=100) # dpi=100 apenas para visualização na tela

    # 2. Plotagem da linha (cores sólidas e clássicas funcionam melhor em artigos)
    ax.plot(
        x, y, linewidth=espessura, color="black", linestyle="-"
    )
    ...
    ax.tick_params(which="major", direction="in", length=6, width=1.2, top=True, right=True)
